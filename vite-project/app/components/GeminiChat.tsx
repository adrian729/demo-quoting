...
const GeminiChat = () => {
  ...
  // We store the raw file object + base64 string
  ...
  const handleFileSelect = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    // Clear value so same file can be re-selected if needed
    e.target.value = "";
  };
  ...
  const fetchResponse = async () => {
    ...
    try {
      ...
      // 1. Add File Part (if exists)
      if (selectedFile) {
        parts.push({
          inlineData: {
            data: selectedFile.base64,
            mimeType: selectedFile.file.type || "text/plain", // Fallback for csv/txt
          },
        });
      }
      ...
      // 3. Generate Content with both parts
      ...
      setError(
        "Failed to generate response. Ensure the model supports this file type.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="mb-6 w-full rounded-lg border border-slate-700 bg-slate-800 p-6 shadow-lg">
      ...
      <div className="flex flex-col gap-4">
        ...
        {selectedFile && (
          <div className="flex items-center justify-between rounded-md border border-emerald-800 bg-emerald-900/20 px-3 py-2">
            <div className="flex items-center gap-2 overflow-hidden">
              ...
              <span className="max-w-[200px] truncate text-sm text-emerald-300 sm:max-w-xs">
        ...
        <textarea
          ...
          rows={3}
          className="w-full resize-y rounded-lg border border-slate-600 bg-slate-900 p-3 text-sm text-slate-200 placeholder-slate-500 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none"
        />
        ...
        {response && (
          <div className="mt-2 rounded-lg border border-slate-700 bg-slate-900/50 p-4">
