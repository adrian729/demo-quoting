...
import { generateContentWithFallback, isAiEnabled } from "~/utils/geminiApi";
        ...
        {!isAiEnabled ? (
          <div className="flex h-full flex-col items-center justify-center gap-2 text-center text-slate-500">
            ...
            <p className="max-w-[200px] text-xs">
              Please configure your API key in the .env file to enable Gemini.
            </p>
          </div>
        ) : (
      ...
      <div className="shrink-0 border-t border-slate-700 bg-slate-800 p-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-lg border border-slate-600 bg-slate-700 text-slate-400 transition-colors hover:bg-slate-600 hover:text-white disabled:cursor-not-allowed disabled:opacity-50"
          >
          ...
          <input
            ...
            placeholder={
              isAiEnabled
                ? "Ask Gemini to edit or analyze..."
                : "AI is currently disabled."
            }
            disabled={loading || !isAiEnabled}
            className="flex-1 rounded-lg border border-slate-600 bg-slate-900 px-4 text-sm text-white placeholder-slate-500 focus:border-blue-500 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
          />
          <button
            onClick={handleSendMessage}
            disabled={
              loading || !isAiEnabled || (!prompt && selectedFiles.length === 0)
            }
